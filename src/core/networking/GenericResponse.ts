import { HttpClientConfig } from "@/core/networking/HttpClient";

interface HttpResponseConfig<T> {
    status: number;
    message: string;
    data: T;
    headers: Record<string, string>;
    requestConfig: HttpClientConfig | null;
}
...
export class GenericResponse<T = Record<string, unknown>> {
     ...
     */
    protected readonly config: HttpResponseConfig<T> = {
        status: 200,
